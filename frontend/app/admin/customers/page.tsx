    ...
    <div className="space-y-12">
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-6">
        <div className="space-y-2">
          <h2 className="text-4xl font-serif font-bold tracking-tight text-stone-900 dark:text-white">Customer Relationship</h2>
          <p className="text-stone-500 font-medium">Manage your community, view purchase history, and handle account permissions.</p>
        </div>
        <button className="flex items-center space-x-2 px-6 py-3 rounded-2xl bg-stone-900 dark:bg-white text-white dark:text-stone-950 hover:bg-stone-800 dark:hover:bg-stone-200 transition-all text-sm font-bold uppercase tracking-widest shadow-xl shadow-stone-200/50 dark:shadow-none">
          <UserPlus size={16} />
      ...
      <div className="space-y-8">
        <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
          ...
          <button className="flex items-center space-x-2 px-6 py-3 rounded-2xl bg-stone-100 dark:bg-stone-900/50 border border-stone-200 dark:border-stone-800 text-stone-600 dark:text-stone-400 hover:text-stone-900 dark:hover:text-white transition-all text-sm font-bold uppercase tracking-widest">
            <Filter size={16} />
            <span>Filter</span>
          </button>
        </div>
          ...
          <div className="overflow-x-auto" data-lenis-prevent="true">
            <table className="w-full text-left border-collapse">
              ...
              <tbody className="divide-y divide-stone-200 dark:divide-stone-800/30">
                {customers.map((user) => (
                  <tr key={user.id} className="group hover:bg-stone-50/50 dark:hover:bg-white/[0.02] transition-colors">
                    <td className="px-8 py-6">
                      <div className="flex items-center space-x-4">
                        <div className="w-10 h-10 rounded-xl bg-stone-100 dark:bg-stone-800 border border-stone-200 dark:border-stone-700 overflow-hidden">
                          {user.image ? (
                        ...
                        <div>
                          <p className="text-sm font-bold text-stone-900 dark:text-white">{user.name || 'Unnamed'}</p>
                          <p className="text-[10px] text-stone-500 uppercase tracking-widest mt-0.5">{user.email}</p>
                        </div>
                      </div>
                    </td>
                    <td className="px-6 py-6 text-sm font-medium text-stone-600 dark:text-stone-300">
                    ...
                    <td className="px-8 py-6 text-right">
                      <div className="flex items-center justify-end space-x-2 opacity-0 group-hover:opacity-100 transition-opacity">
                        <button className="p-2 rounded-lg hover:bg-stone-100 dark:hover:bg-stone-800 text-stone-500 hover:text-stone-900 dark:text-stone-400 dark:hover:text-white transition-all">
                          <Mail size={16} />
                        </button>
                        <button className="p-2 rounded-lg hover:bg-stone-100 dark:hover:bg-stone-800 text-stone-500 hover:text-stone-900 dark:text-stone-400 dark:hover:text-white transition-all">
                          <ShieldCheck size={16} />
                        </button>
                        <button className="p-2 rounded-lg hover:bg-red-500/10 text-stone-500 hover:text-red-500 transition-all">
                          <Ban size={16} />
                        </button>
                      </div>
                    </td>
