...
import { getUserAddresses } from '@/app/actions/settings';
  ...
  return (
    <>
      <header className="fixed top-0 left-0 w-full z-50 flex flex-col">
                  ...
                  {isProfileOpen && session && (
                    <>
                      ...
                      <motion.div
                        ...
                        className="absolute right-0 mt-2 w-56 bg-stone-900 border border-stone-850 rounded-xl shadow-2xl p-2 z-50 text-white"
                      >
        ...
        <div className="w-full bg-stone-800 dark:bg-stone-900 border-b border-stone-850 py-1.5 px-4 md:px-6 text-stone-300 flex items-center justify-between text-xs overflow-x-auto no-scrollbar font-semibold">
          <div className="container mx-auto flex items-center justify-between w-full">
            ...
            <div className="hidden lg:flex items-center space-x-4 shrink-0 text-stone-400 text-[10px] tracking-wider uppercase font-bold">
      ...
      <AnimatePresence>
        {isMobileMenuOpen && (
          <motion.div
            ...
            exit={{ opacity: 0, y: -20 }}
            className="md:hidden fixed top-[86px] left-0 w-full bg-stone-900 text-white border-b border-stone-850 z-40 p-6 flex flex-col space-y-4 shadow-2xl"
          >
        ...
        {isPincodeModalOpen && (
          <>
            ...
            <motion.div
              ...
              className="fixed top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-[90%] max-w-md bg-white dark:bg-stone-900 border border-stone-200 dark:border-stone-800 p-6 rounded-2xl shadow-2xl z-[100] text-stone-900 dark:text-white space-y-5"
            >
              <div className="flex justify-between items-center border-b border-stone-100 dark:border-stone-800 pb-3">
                <h3 className="text-sm font-black uppercase tracking-wider">Choose your location</h3>
                <button onClick={() => setIsPincodeModalOpen(false)} className="text-stone-450 hover:text-stone-900 dark:hover:text-white">
                  <X size={16} />
                </button>
              </div>

              {/* Saved Addresses List (Amazon Style) */}
              {session ? (
                      ...
                      <button
                        ...
                        className={`w-full text-left p-3 border rounded-xl transition-colors flex items-start space-x-3 text-xs ${
                          pincode === addr.postalCode 
                            ...
                            : 'border-stone-200 dark:border-stone-850 hover:bg-stone-50/50 dark:hover:bg-stone-850/50'
                        }`}
                      >
                        <MapPin size={14} className="text-stone-400 shrink-0 mt-0.5" />
                        <div>
                          <p className="font-bold dark:text-white truncate">{addr.street}</p>
                          <p className="text-stone-500 dark:text-stone-400">{addr.city}, {addr.state} - {addr.postalCode}</p>
                        </div>
                      </button>
                    ))
                  ) : (
                    <p className="text-xs text-stone-500 italic py-2">No saved addresses found. Enter a PIN code below.</p>
                  )}
                </div>
              ) : (
              ...
              {/* Enter PIN Code Input (Amazon Style) */}
              <div className="space-y-2 pt-2 border-t border-stone-100 dark:border-stone-800">
                ...
                <form onSubmit={handlePincodeSubmit} className="flex gap-2">
                  ...
                  <button
                    type="submit"
                    className="px-4 py-2 bg-stone-900 dark:bg-white text-white dark:text-stone-900 text-xs font-bold uppercase rounded-lg hover:opacity-90 transition-opacity"
                  >
                    Apply
                  </button>
                </form>
              ...
              {/* International Country Select (Amazon Style) */}
              <div className="space-y-2 pt-2 border-t border-stone-100 dark:border-stone-800">
                ...
                <select
                  ...
                  className="w-full px-3 py-2 text-xs border border-stone-300 dark:border-stone-800 rounded-lg bg-transparent dark:text-white outline-none cursor-pointer"
                >
